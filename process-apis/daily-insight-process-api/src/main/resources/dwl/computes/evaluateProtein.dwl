%dw 2.0
fun evaluateProtein(loggedProteinG, resolvedTarget) =if ( loggedProteinG == null ) {
	metric_key: "protein_g",
	status: "no_data",
	notes: "logged ProteinG was not be computed"
}else do {
	...
	{
		metric_key: "protein_g",
		logged_value_g: loggedProteinG as Number {
			format: "0.#"
		},
		target_g: resolvedTarget.target_g,
		hard_bound_g: resolvedTarget.hard_bound_g,
		pct_of_target: pctOfTarget as Number {
			format: "0.#"
		},
		status: status,
