...
// This row is NOT a Daily Insight dimension status (activity has none).
// It exists purely to feed cross-signal rules 2 and 3 (low calories/protein +
// high active energy), which need a "was today high?" check that has no other
// source of truth.
  {
	threshold_id: "active_energy_baseline_deviation",
	dimension: "activity",
	...
	soft_bound_low: -0.40,
	hard_bound_low: -0.60,
	soft_bound_high: 0.50,
	hard_bound_high: 0.75,
	unit: "kcal",
	baseline_window_days: 30,
	baseline_statistic: "average",
	min_data_days: 14,
	...
	source_url: null,
	confidence: "heuristic",
	notes: "Bounds are deliberately asymmetric and wider than HR/HRV, since a single hard training day or single rest day is normal variance against a 30-day average that already includes both. Lower side (-40%/-60%) flags sustained sedentary drift, not one rest day. Upper side (+50%/+75%) flags high exertion, intentionally looser since the co-occurring nutrition shortfall in the cross-signal pairing (rules 2 and 3) does most of the actual filtering — this bound alone was never meant to stand on its own."
}]
/**
 ...
 */
var knownLimitations = ["Flat percentage bands do not adapt to individual variability.",
 ...
 * Bidirectional rows are nOT used to produce a Daily Insight dimension status on their own.
 * they exist to feed cross-signal detection, which needs to ask "was today unusually high/low relative
 * to this person's own recent mix of active and rest days. 
 */
fun resolveBaselineBounds(metricKey, baselineValue) = do {
	var row = baselineRowFor(metricKey)
	---
	if ( row == null or baselineValue == null ) null
  else if ( row.direction == "bidirectional" ) {
		...
		soft_bound_low: (baselineValue * (1 + row.soft_bound_low)) as Number {
			format: "0.#"
		},
		hard_bound_low: (baselineValue * (1 + row.hard_bound_low)) as Number {
			format: "0.#"
		},
		soft_bound_high: (baselineValue * (1 + row.soft_bound_high)) as Number {
			format: "0.#"
		},
		hard_bound_high: (baselineValue * (1 + row.hard_bound_high)) as Number {
			format: "0.#"
		},
		...
		baseline_statistic: row.baseline_statistic,
		baseline_window_days: row.baseline_window_days,
		soft_bound: (baselineValue * (1 + row.soft_bound)) as Number {
			format: "0.#"
		},
		hard_bound: (baselineValue * (1 + row.hard_bound)) as Number {
			format: "0.#"
		},
		confidence: row.confidence,
		source_body: null,
